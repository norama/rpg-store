...
export const stringArraysMap = map<Record<string, string[]>>({})
...
PubSub.subscribe(M.uiStringArray, (_msg: string, { key, value }: IDataStringArray) => {
...
export const setStringArray = (key: string, value: string[]) => {
  stringArraysMap.setKey(key, value)
}

export const save = (properties?: Record<string, IValue>) => {
  const block = { ...stringValuesMap.get(), ...numberValuesMap.get(), ...stringArraysMap.get() }
